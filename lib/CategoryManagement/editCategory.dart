...
class _EditCategoryState extends State<EditCategory> {
                        ...
                        child: Column(
                          children: <Widget>[
                            ...
                            ElevatedButton(
                                ...
                                child: BigText(text: "Update Category")),
                            SizedBox(
                              height: 20,
                            ),
                            ElevatedButton(
                                ...
                                child: BigText(text: "Delete Category")),
                          ],
                        ),
                      ),
                    ),
  ...
  Future<void> getData() async {
    if (widget.exp > _amount.toDouble()) {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Alert'),
            content: Text("Your Budget Can't be less than your current spend"),
            actions: [
              ElevatedButton(
                onPressed: () {
                  setState(() {
      ...
      final Category other = Category(
          ...
          expense: widget.exp,
          icon: txt1,
          budget: _amount.toDouble());

      print(other.icon);

      List<Category> savedCategories = await CategoryStorage.getCategories();
      savedCategories.removeAt(widget.index);
      savedCategories.insert(widget.index, other);

      await CategoryStorage.saveCategories(savedCategories);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Alert'),
          content: Text(
              "Are you sure you want to delete this Category? Deleting this would remove all expences in this category as well"),
          actions: [
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  _delete() async {
    ...
    double exp = prefs.getDouble('expence')!;

    List<Expense> updatedExpenses = [];

    for (Expense ex in existingExpenses) {
      if (ex.type != widget.name) {
        updatedExpenses.add(ex);
      } else {
        exp = exp - ex.amount;
        await prefs.setDouble('expence', exp);
      }
    }

    existingExpenses = updatedExpenses;
